...
function getTarget(db, opts, callback) {
  ...
  db.hgetall('contact-stats', function(err, stats) {
    ...
    if (stats.timestamp > now + retargetPeriod) {

      const timeDelta = now - stats.timestamp;
      const actual = status.count;

      const expectedRatio = retargetPeriod / count;
      const actualRatio = timeDelta / actual;

      const adjustment = new BN(expectedRatio / actualRatio);
      const target = new BN(status.target, 16);
      const newTarget = target.mul(adjustment).toString(16);
...
function getChallenge(db, opts, callback) {
  getTarget(db, opts, function(err, target) {
    if (err) {
      return callback(err);
    }
